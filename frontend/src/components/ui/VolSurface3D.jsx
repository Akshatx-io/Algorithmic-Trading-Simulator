 ...
 * Manual 3D->2D projection (yaw/pitch), painter's-algorithm depth sort, per-quad
 * Lambert shading + turbo colormap. Drag to rotate; gentle auto-spin when idle.
 * ~150 quads/frame => trivially 60fps.
 */
const HEIGHT = 0.62;
const LIGHT = (() => { const v = [0.4, -0.5, 0.75]; const m = Math.hypot(...v); return v.map((c) => c / m); })();

export default function VolSurface3D({ iv = [], zmin = 0, zmax = 1, autoRotate = true, height = 460 }) {
  ...
  const draw = () => {
    ...
    const P = (i, j) => {
      ...
      const zv = (iv[j][i] - zmin) / span;
      return { x, y, z: (zv - 0.5) * HEIGHT, zv };
    };
    ...
    for (let j = 0; j < ny - 1; j++) {
      for (let i = 0; i < nx - 1; i++) {
        ...
        // world normal (z = height) for stable lighting of the shape
        ...
        const shade = 0.45 + 0.55 * Math.max(0, nxn * LIGHT[0] + nyn * LIGHT[1] + nzn * LIGHT[2]);
        const pa = project(a.x, a.y, a.z, cx, cy, scale);
        const pb = project(b.x, b.y, b.z, cx, cy, scale);
        const pd = project(d.x, d.y, d.z, cx, cy, scale);
        const pe = project(e.x, e.y, e.z, cx, cy, scale);
        quads.push({
          pts: [pa, pb, pd, pe],
    ...
    // colorbar
    const bw = 10, bh = H * 0.5, bx = W - 26, by = H * 0.22;
    for (let t = 0; t < bh; t++) {
  ...
  const onMove = (e) => {
    ...
    pitch.current = Math.max(-1.45, Math.min(0.2, pitch.current + (e.clientY - drag.current.y) * 0.008));
    drag.current = { x: e.clientX, y: e.clientY };
    draw();
  };
