...
export default function MonteCarloViz({
  ...
  const drawPaths = (u) => {
    ...
    const bg = ctx.createLinearGradient(0, MT, 0, H - MB);
    bg.addColorStop(0, "rgba(56,189,248,0.06)");
  ...
  const renderFrame = (u) => {
    ...
    if (barRef.current) barRef.current.style.width = `${clamp01(u) * 100}%`;
    const cf = N ? done / N : 0;
    drawHist(cf);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-4">
          <div className="rounded-lg border border-line/60 bg-ink-900/60 px-3 py-1.5 text-right">
      ...
      <div className="h-1 w-full overflow-hidden rounded-full bg-ink-900">
        <div ref={barRef} className="h-full rounded-full bg-brand-gradient" style={{ width: 0 }} />
      </div>
